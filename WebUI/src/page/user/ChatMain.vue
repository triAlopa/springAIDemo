...
import {userQuerySessionApi, userDeleteSessionApi, userCreateSessionApi, userSessionReteApi} from '@/api/session.js';
...
/*            console.log(currentBaseInfo.value)
            ...
            console.log(isSessionDone.value)*/
...
const handleSessionRate = async (rate) => {
  console.log(rate)
  await userSessionReteApi(currentSessionId.value, rate).then(result => {
    if (result.code === 200) {
      ElMessage.success('打分成功,感谢使用我们产品')
    } else {
      console.log(result.msg)
    }
  }).catch(Error => {
...
<template>
  ...
  <div class="h-screen w-full flex flex-col justify-center items-center overflow-hidden relative">
    ...
    <div
        class="absolute bottom-[-10%] right-[-10%] w-[400px] h-[400px] bg-purple-300/30 rounded-full blur-[100px] pointer-events-none animate-float"
    ...
    <transition name="el-fade-in-linear" mode="out-in">
      ...
      <div
        ...
        <the-chat-window :messages="currentMessages" :current-chat-title="currentChatTitle"
                         :currentBaseInfo="currentBaseInfo" @send-message="handleSendMessage"
                         :currentModel="currentModel"
                         :isSessionDone="isSessionDone"
                         :is-dark="isDark" @handleOffer="handleOfferClick" @session-rate="handleSessionRate">
          <!-- 右上角的胶囊  -->
